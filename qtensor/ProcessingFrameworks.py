...
class NumpyBackend(BucketBackend):
    ...
    def process_bucket(self, bucket, no_sum=False):
        res = np_framework.process_bucket_jnp(bucket, no_sum=no_sum)
        return res

    def get_sliced_buckets(self, buckets, data_dict, slice_dict):
        return np_framework.get_sliced_jnp_buckets(buckets, data_dict, slice_dict)
...
class CMKLExtendedBackend(BucketBackend):
    ...
    def process_bucket(self, bucket, no_sum=False):
        ...
        def merge_with_result(result_data, result_indices, tensor):
            # ---- Prepare ijnputs: transpose + reshape
            ixa, ixb = result_indices, tensor.indices
            common_ids = sorted(
                list(set.intersection(set(ixa), set(ixb))), key=int)
            ...
            c = jnp.empty((n, m, k), dtype=jnp.complex128)
            ...
            assert len(result_indices) == len(
                ixc), 'Wrong transposition, please submit an issue'
        ...
        for tensor in bucket[1:-1]:
            result_data, result_indices = merge_with_result(
                result_data, result_indices, tensor)
        # --
        ...
        if no_sum:
            if len(bucket) > 1:
                ...
                result_data, result_indices = merge_with_result(
                    result_data, result_indices, last_tensor)
        ...
        if len(bucket) < 2:
            result = opt.Tensor(f'E{tag}', result_indices[1:],
                                data=jnp.sum(result_data, axis=0))
            return result
        last_tensor = bucket[-1]
        ...
        K, F, M, N = [reduce(jnp.multiply, (shapes[i]
                             for i in x), 1) for x in (k, f, m, n)]
        a = a.reshape(K, F, M)
        b = b.reshape(K, F, N)
        # ----

        # \sum_k A_{kfm} * B_{kfn} = C_{fmn}
        c = jnp.empty((F, M, N), dtype=jnp.complex128)
...
class PerfBackend(BucketBackend):
    ...
    def __init__(self, *args, print=False, num_lines=20, **kwargs):
        ...
        self.report_table = ReportTable(
            measure=['max', 'mean', 'sum'], max_records=num_lines)
    ...
    def process_bucket(self, bucket, no_sum=False):
        ...
        with timing('process bucket time', indices, callback=self._profile_callback):
    ...
    def _perfect_bucket_flop(self, bucket_indices):
        resulting_indices = list(
            set.union(*[set(ixs) for ixs in bucket_indices]))
    ...
    def gen_report(self):
        ...
        report_lines = [str([i, ixs, time])
                        for i, (ixs, time) in enumerate(data[:max_lines])]
        ...
        for indices, time in data[:max_lines]:
            self.report_table.record(
                bucket_len=len(indices), time=time, flop=self._perfect_bucket_flop(indices), FLOPS=self._perfect_bucket_flop(indices)/time, max_size=max([len(ixs) for ixs in indices]), min_size=min([len(ixs) for ixs in indices]), result_size=len(set.union(*[set(i) for i in indices])) - 1
            )
